...
namespace VCore
{
    ...
    public class VideoRecorder : MonoBehaviour
    {
        ...
        [Header("Upload")]
        [Tooltip("Shared backend address. When assigned it overrides the base URL below.")]
        public BackendConfig backendConfig;

        [Tooltip("Fallback V-CORE base URL, used only when no Backend Config asset is assigned.")]
        public string vcoreBaseUrl = "http://localhost:8000";

        private string ResolvedBaseUrl =>
            backendConfig != null ? backendConfig.HttpBaseUrl : vcoreBaseUrl;
        ...
        private IEnumerator UploadMetadata()
        {
            ...
            var endpoint = $"{ResolvedBaseUrl.TrimEnd('/')}/api/sessions/{_sessionId}/recording";
